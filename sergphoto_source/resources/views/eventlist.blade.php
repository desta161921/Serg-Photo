    ...
    <div class="container__events">
        <div class="wrapper__eventlist">
            ...
            <div class="event__info">
                ...
                @foreach ($eventCount as $i)
                    @if(\App\Event::find($i->event_Id)->event_Privacy == 'public')
                        <tr>
                            <td><p class="event__description-name el__d-name"><a class="event__description-link" href="{{ route('events')}}/{{$i->event_Id}}">{{\App\Event::find($i->event_Id)->event_Name}}</a></p></td>
                            <td><p class="event__description-about">{{str_limit(\App\Event::find($i->event_Id)->event_Description, 50) }}</p></td>
                            <td><p class="event__description-about">{{\App\Event::find($i->event_Id)->event_Location}}</p></td>
                        </tr>
                    @endif
                @endforeach
                </table>
